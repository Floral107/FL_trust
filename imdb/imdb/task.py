...
path_to_save = os.path.join(BASE_DIR.parent, "420", "imdb")
...
def load_model():
    ...
    # LR 2e-5 -> 1e-3 (2026-07-08): see imdbnoniid/task.py — convergence fix.
...
def load_data(partition_id, num_partitions):
    set_seed(SEED)
    global fds
    # Only initialize the dataset once
    if fds is None:
...
def process_text(text_list):
    """Embed text with USE in small chunks to avoid OOM spikes.

    Returns a float32 numpy array of shape (N, 512). Processing all texts in
    one shot allocates huge intermediate tensors inside USE; chunking keeps
    peak memory proportional to _USE_EMBED_CHUNK rather than the full dataset.
    trainable=False saves gradient-tracking state for USE's ~256M parameters.
    """
    import gc
    # Idempotent: already-embedded (N,512) float input passes straight through,
    # so callers can embed ONCE and reuse across many coalition evals.
