...
def on_evaluate(server_round, parameters, config):
    ...
    save_global_model(model, client_num, server_round,
...
def server_fn(context):
    ...
    _ET_CONFIG = {
        ...
        "et_adv_eps":     float(context.run_config.get("et-adv-eps", 0.03)),
    ...
    client_num = get_num_clients()
    model = load_model()
    save_global_model(model, client_num, 0,
    ...
    if et_mode in ("fair", "adv", "dp"):
        ...
        # This package's own load_data, 6-tuple signature (as eval uses).
        # Not reweight_eval.load_test_data (root-relative imports break here).
        _, _, x_test, y_test, _, s_test = load_data(0, 1, return_sensitive=True, test_only=False)
        metric_fn = build_metric(weight_metric, x_test, y_test, s_test=s_test,
                                 dataset="celebanoniid")
